...
import com.example.storymapapp.api.ApiConfig
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun addFile(
        ...
        val service =
            ApiConfig.getApi()
                .uploadImage(imageMultipart, description, lat, lon, token = "Bearer $token")
    ...
    fun setLocation(token: String) {
        ...
        val setLocation = ApiConfig.getApi().getStoryMap(1, token = "Bearer $token")
        setLocation.enqueue(object : Callback<StoryMap> {
            override fun onResponse(
                call: Call<StoryMap>,
                response: Response<StoryMap>,
            ) {
            ...
            override fun onFailure(call: Call<StoryMap>, t: Throwable) {
                _isLoading.value = false
                Log.d("Failure", t.message.toString())
            }
        })
